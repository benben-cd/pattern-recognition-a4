...
def setup_chinese_font():
    ...
    # 如果没有找到中文字体，尝试下载Noto Sans CJK
    return False, None

# 设置字体
has_chinese_font, selected_font = setup_chinese_font()
...
def get_class_names():
    """返回类别名称列表（中文或英文）"""
    return CIFAR10_CLASSES_CN if has_chinese_font else CIFAR10_CLASSES_EN
...
def download_and_load_cifar10():
    ...
    try:
        ...
        return (X_train, y_train), (X_test, y_test), get_class_names()
...
def generate_linear_classifier_weights(n_classes=10):
    ...
    return weights, get_class_names()
...
def least_squares_module():
    ...
    with col1:
        ...
        regularization = st.slider("L2 Regularization (λ)", 0.0, 1.0, 0.0, 0.01, key="ls_reg")
...
def knn_module():
    ...
    with col_dl1:
        if not has_real_data:
            st.warning("⚠️ Real CIFAR-10 data not downloaded (~170MB)")
            ...
            if has_real_data and use_real_data:
                ...
                for i, idx in enumerate(indices[0][:5]):
                    with cols[i % 5]:
                        ...
                        true_label = class_names[y_train_real[idx]]
                        st.image(neighbor_img, caption=f"{true_label}\nDist:{distances[0][i]:.1f}", width=80)
            else:
                st.info("Using simulated data for classification (based on color features)")
                ...
                st.success(f"**Predicted Class: {class_names[predicted_class]}** (based on color similarity)")
                st.write(f"Image Color Features: R={r_mean:.1f}, G={g_mean:.1f}, B={b_mean:.1f}")
    ...
    with col2:
        if use_real_data:
            ...
            for idx, class_id in enumerate(selected_classes[:5]):
                with cols[idx]:
                    ...
                    if len(class_images) > 0:
                        st.image(class_images[0], caption=class_names[class_id], width=100)
        else:
            # 使用模拟数据
            ...
            legend_elements = [Patch(facecolor=np.array(class_color_hints[c])/255, edgecolor='k', 
                                   label=class_names[c]) for c in selected_classes]
...
def linear_classifier_templates_module():
    ...
    st.info("""
    **Note:** The following templates are hand-designed simulated templates for demonstrating the visualization principle of linear classifiers.
    Real CIFAR-10 linear classifier templates are obtained by training on 50,000 images.
    """)
    
    weights, _ = generate_linear_classifier_weights(n_classes=10)
    ...
    **Template Interpretation:**
    ...
    - Decision rule is wᵀx: if wᵢ is large, then large xᵢ values are indicative of the class
    - During classification, compute dot product (similarity) between input image and each template
...
def sgd_momentum_module():
    ...
    with col1:
        ...
        momentum = st.slider("Momentum (ρ)", 0.0, 0.99, 0.9, 0.01)
...
def loss_demo_module():
    ...
    if loss_type == "Mean Squared Error (MSE)":
        ...
        with col1:
            ...
            y_pred = st.slider("Predicted Value ŷ", -5.0, 5.0, 1.0, 0.1)
        ...
        with col2:
            ...
            ax1.set_xlabel('Predicted Value ŷ')
        ...
        with col1:
            ...
            delta = st.slider("Margin Δ", 0.0, 3.0, 1.0, 0.1)
...
def main():
    ...
    if not has_chinese_font:
        st.info("ℹ️ Chinese font not detected. The interface will use English labels for better compatibility.")
    ...
    module = st.sidebar.radio(
        ...
        [
            ...
            "🔍 KNN Comparison",
            "🖼️ CIFAR Linear Classifier Templates",
            "🚀 SGD vs Momentum",
            "📊 Loss Function Demo"
        ]
    )
    ...
    if module == "🏠 Home":
        ...
        4. **🖼️ CIFAR Linear Classifier Templates**
